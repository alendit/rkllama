...
from rkllama.api.format_utils import (
    strtobool,
    openai_to_ollama_chat_request,
    openai_to_ollama_generate_request,
    normalize_openai_format_spec,
)
...
def load_model(
    ...
    if not request_options:
        request_options = get_model_full_options(
            model_name, rkllama.config.get_path("models"), request_options
        )
    ...
    if not model_loaded:
        return (
            None,
            f"Unexpected Error loading the model {model_name} into memory. Check the file .rkllm is not corrupted, properties in Modelfile (like Context Length allowed by the model) and resources available in the server",
        )
    else:
        return None, None
...
def list_models():
    ...
    if not os.path.exists(models_dir):
        return (
            jsonify({"error": f"The models directory {models_dir} is not found."}),
            500,
        )
...
def load_model_route():
    ...
    if model_name is None:
        return (
            jsonify({"error": "Please enter the name of the model to be loaded."}),
            400,
        )
...
def unload_model_route():
    ...
    if model_name is None:
        return (
            jsonify({"error": "Please enter the name of the model to be unloaded."}),
            400,
        )
...
def get_current_models():
                ...
                if file.endswith(".rkllm"):
                    ...
                    models_info[subdir] = {
                        ...
                        "details": {
                            ...
                            "quantization_level": model_details.get(
    ...
    for model in variables.worker_manager_rkllm.workers.keys():
        worker_model_info = variables.worker_manager_rkllm.workers[
            model
        ].worker_model_info
        model_info = {
            "name": model,
            ...
            "digest": (
                models_info[model]["digest"] if model in models_info.keys() else None
            ),
            "details": {
                "parent_model": "",
                "format": (
                    models_info[model]["details"]["format"]
                    if model in models_info.keys()
                    else "rknn"
                ),
                "family": (
                ...
                "parameter_size": (
                    models_info[model]["details"]["parameter_size"]
                    if model in models_info.keys()
                    else None
                ),
                "quantization_level": (
                    models_info[model]["details"]["quantization_level"]
                    if model in models_info.keys()
                    else None
                ),
            },
    ...
    for subdir in os.listdir(models_dir):
        ...
        if os.path.isdir(subdir_path):
            for file in os.listdir(subdir_path):
                ...
                ):  # Include Stable Diffusion models and other rknn models
                    models.append(
                        {
                            "id": subdir,
                            "object": "model",
...
def list_openai_model(model_name):
    ...
    for subdir in os.listdir(models_dir):
        ...
        if os.path.isdir(subdir_path):
            for file in os.listdir(subdir_path):
                if (
                    file.endswith(".rkllm") or file == "unet"
                ):  # Include Stable Diffusion models
...
def show_model_info():
    ...
    model_name = (
        re.search(r"/(.*)", model_name).group(1)
    ...
    if not model_file:
        return (
            jsonify({"error": f"Model file not found in '{model_name}' directory"}),
            404,
        )
    ...
    if hf_metadata:
        response["huggingface"] = {
            ...
            "description": (
                model_description[:500] if model_description else ""
...
def create_model():
    ...
    if not from_line or not huggingface_path:
        return (
            jsonify({"error": "Invalid Modelfile: missing FROM or HUGGINGFACE_PATH"}),
            400,
        )
    ...
    try:
        ...
        if enable_thinking is None:
            model_thinking_enabled = get_property_modelfile(
                model_name, "ENABLE_THINKING", rkllama.config.get_path("models")
            )
            enable_thinking = (
        ...
        options = get_model_full_options(
            model_name, rkllama.config.get_path("models"), options
        )
        ...
        if not variables.worker_manager_rkllm.exists_model_loaded(model_name):
            ...
            if error:
                return (
                    jsonify({"error": f"Failed to load model '{model_name}': {error}"}),
                    500,
                )
...
def responses_ollama():
    ...
    try:
        ...
        format_spec = (
            data.get("text", {}).get("format")
            if isinstance(data.get("text"), dict)
            else data.get("format")
        )
        format_spec = normalize_openai_format_spec(format_spec)
        options = data.get("options", {})
        images = data.get("images", None)
...
def embeddings_ollama():
    ...
    try:
        ...
        )  # Include legacy deprecated api/embedding with prompt
...
def ollama_version():
    ...
    return jsonify({"version": "0.0.44"}), 200
...
def main():
    ...
    parser.add_argument(
        "--models", type=str, help="Path whe models will be loaded from"
    )
    ...
    if not processor:
        ...
        if os.getuid() == 0:
            print_color(
                f"Setting the frequency for the {processor} platform...", "cyan"
            )
            library_path = (
                importlib.resources.files("rkllama.lib") / f"fix_freq_{processor}.sh"
            )
