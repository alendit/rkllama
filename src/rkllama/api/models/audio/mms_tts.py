...
class MMSTTSModelRKNN:
    ...
    def run_encoder(self, input_ids_array, attention_mask_array):
        log_duration, input_padding_mask, prior_means, prior_log_variances = (
            self.encoder_rknn.inference(inputs=[input_ids_array, attention_mask_array])
        )
    ...
    def run_decoder(self, attn, output_padding_mask, prior_means, prior_log_variances):
        waveform = self.decoder_rknn.inference(
    ...
    def generate_speech(
        ...
        log_duration, input_padding_mask, prior_means, prior_log_variances = (
            self.run_encoder(input_ids_array, attention_mask_array)
        )
        logger.debug(f"Encoder time: {(time.time() - encoder_start):.2f}s")

        # Middle process
        attn, output_padding_mask, predicted_lengths_max_real = self.middle_process(
            log_duration, input_padding_mask
        )

        # Decode
        decoder_start = time.time()
        waveform = self.run_decoder(
