...
class OmniCtcSTTModelRKNN:
    ...
    def __call__(self, x: np.ndarray, chunk_len: int):
        ...
        logits = self.trim_rknn_asr_output(
            logits, real_input_len=x.shape[1], static_input_len=padded_x.shape[1]
        )
    ...
    def split_audio(self, samples, overlap_sec=1.0):
        ...
        logger.debug(
            f"chunk_len: {chunk_len} | overlap: {overlap} | step: {step}  | samples: {len(samples)} "
        )
