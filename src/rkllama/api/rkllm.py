...
class RKLLM(object):
    def __init__(
        ...
        self.rkllm_param.max_context_len = int(
            float(
                (options.get("num_ctx", rkllama.config.get("model", "default_num_ctx")))
            )
        )
        ...
        self.rkllm_param.top_k = float(
            options.get("top_k", rkllama.config.get("model", "default_top_k"))
        )
        self.rkllm_param.top_p = float(
            options.get("top_p", rkllama.config.get("model", "default_top_p"))
        )
        ...
        self.rkllm_param.frequency_penalty = float(
            options.get(
                "frequency_penalty",
                rkllama.config.get("model", "default_frequency_penalty"),
            )
        )
        self.rkllm_param.presence_penalty = float(
            options.get(
                "presence_penalty",
                rkllama.config.get("model", "default_presence_penalty"),
            )
        )
        self.rkllm_param.mirostat = int(
            options.get("mirostat", rkllama.config.get("model", "default_mirostat"))
        )
        ...
        if processor.lower() in ["rk3576", "rk3588"]:  # Recommended new way by Rockchip
            self.rkllm_param.extend_param.enabled_cpus_mask = (
                (1 << 4) | (1 << 5) | (1 << 6) | (1 << 7)
            )
        else:
            self.rkllm_param.extend_param.enabled_cpus_mask = (
        ...
        ret = self.rkllm_init(
            ctypes.byref(self.handle), ctypes.byref(self.rkllm_param), callback
        )
        if ret != 0:
        ...
        self.set_function_tools_.argtypes = [
            RKLLM_Handle_t,
            ctypes.c_char_p,
            ctypes.c_char_p,
            ctypes.c_char_p,
        ...
        if all(
            item in options
            for item in ["system_prompt", "prompt_prefix", "prompt_postfix"]
        ):
        ...
        self.rkllm_infer_params.lora_params = (
            ctypes.pointer(self.rkllm_lora_params) if self.rkllm_lora_params else None
        )
    ...
    def run(self, *param):
        ...
        if model_input_type == RKLLMInputType.RKLLM_INPUT_TOKEN:
            ...
            rkllm_input.input_data.token_input.input_ids = ctypes.cast(
                token_array, ctypes.POINTER(ctypes.c_int32)
            )
            rkllm_input.input_data.token_input.n_tokens = ctypes.c_size_t(
                len(token_input)
            )
            ...
            rkllm_input.input_data.embed_input.embed = ctypes.cast(
                embed_array, ctypes.POINTER(ctypes.c_float)
            )
            ...
            logger.debug(
                f"Running multimodal inference with {num_images} images, each of size {image_width}x{image_height}, and {n_image_tokens} image tokens."
            )

            # Prompt
            rkllm_input.input_data.multimodal_input.prompt = prompt_input.encode(
                "utf-8"
            )

            # Image Embedding
            arr = image_embed.flatten().astype(np.float32)
            rkllm_input.input_data.multimodal_input.image_embed = arr.ctypes.data_as(
                ctypes.POINTER(ctypes.c_float)
            )
            rkllm_input.input_data.multimodal_input.n_image_tokens = ctypes.c_size_t(
                n_image_tokens
            )
            rkllm_input.input_data.multimodal_input.n_image = ctypes.c_size_t(
                num_images
            )
            rkllm_input.input_data.multimodal_input.image_width = ctypes.c_size_t(
                image_width
            )
            rkllm_input.input_data.multimodal_input.image_height = ctypes.c_size_t(
                image_height
            )
        ...
        if prompt_cache_file:
            ...
            if os.path.isfile(prompt_cache_file_path):
                self.rkllm_load_prompt_cache(
                    self.handle,
                    ctypes.c_char_p((prompt_cache_file_path).encode("utf-8")),
                )
            else:
                logger.warning(
                    f"Not found expected prompt cache file for fast inference: {prompt_cache_file_path}!!!"
                )

            # Prepare the new cache filename to save
            new_prompt_cache_file = self.increase_prompt_cache_num_messages(
                prompt_cache_file, 2
            )  # Assistant Response + Future User Message
            new_prompt_cache_file_path = (
                f"{self.prompt_cache_dir}/{new_prompt_cache_file}"
            )
            logger.debug(
                f"New prompt cache file will be created after inference: {new_prompt_cache_file}"
            )
            ...
            self.prompt_cache_params.prompt_cache_path = ctypes.c_char_p(
                (new_prompt_cache_file_path).encode("utf-8")
            )
            self.rkllm_infer_params.prompt_cache_params = ctypes.pointer(
                self.prompt_cache_params
            )
